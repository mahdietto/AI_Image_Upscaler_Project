...
// In handleProcessClick function, update fetch:
  ...
  // No headers needed for FormData
    ...
    async function handleProcessClick() {
        ...
        try {
            ...
            if (result.success) {
                // Update comparison slider with enhanced image
                const highImg = document.querySelector('.high-img');
                if (highImg && result.enhanced) {
                    highImg.src = `${API_BASE_URL}${result.enhanced.path}`;
                }
                
                // Show success message
                showNotification('Image successfully enhanced! Drag the slider to compare.', 'success');
                
                // Enable download button
                if (result.enhanced && result.enhanced.downloadUrl) {
                    createDownloadButton(result.enhanced.downloadUrl);
                }
            } else {
            ...
            showNotification(`API Error: ${error.message}. Using simulated enhancement for demonstration.`, 'error');
    ...
    function simulateEnhancement() {
        ...
        highImg.src = canvas.toDataURL('image/jpeg', 0.9);
        
        // Enable download button for simulated image
        const downloadUrl = canvas.toDataURL('image/jpeg');
        createDownloadButtonForDataUrl(downloadUrl);
    }

    // Create download button for data URL
    function createDownloadButtonForDataUrl(dataUrl) {
        const existingBtn = document.querySelector('.download-btn');
        if (existingBtn) existingBtn.remove();
